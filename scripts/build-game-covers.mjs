// Builds src-tauri/resources/game_covers.pack: downloads each catalog game's cover art,
// converts to 160px WebP, and packs into one ~30MB file read lazily from disk (not
// include_bytes!-embedded). Format: [u32 LE index length][index JSON][blob bytes].
...
const OUT = path.join(ROOT, "src-tauri", "resources", "game_covers.pack");
...
const TMP = fs.mkdtempSync(path.join(os.tmpdir(), "capcove-covers-"));

const CONCURRENCY = 16;
const SIZE = 160;

const catalog = JSON.parse(fs.readFileSync(CATALOG, "utf8"));
// One cover per display name (multiple exes share a game entry).
const targets = new Map();
for (const app of catalog) {
  if (app.id && app.cover_image_hash && !targets.has(app.name)) {
    targets.set(app.name, `https://cdn.discordapp.com/app-icons/${app.id}/${app.cover_image_hash}.png?size=256`);
  }
}
console.log(`games with covers: ${targets.size}`);
...
async function processOne(name, url, slot) {
  const pngPath = path.join(TMP, `c${slot}.png`);
  const webpPath = path.join(TMP, `c${slot}.webp`);
  try {
...
console.log(`packed ${results.size} covers (${failed} failed) -> ${OUT} (${(fs.statSync(OUT).size / 1048576).toFixed(1)} MB)`);
