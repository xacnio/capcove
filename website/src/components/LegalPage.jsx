  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-6xl mx-auto px-6 py-20 w-full">
        ...
        {doc.updated && <p className="mt-3 text-sm text-stone-500">Last updated: {doc.updated}</p>}
      ...
      {/* `Footer` has no `w-full` of its own (unlike `main` above) — inside
          this flex column it doesn't stretch to the container's width on its
          own, unlike on the plain (non-flex) index page, so it needs an
          explicit full-width wrapper here. */}
